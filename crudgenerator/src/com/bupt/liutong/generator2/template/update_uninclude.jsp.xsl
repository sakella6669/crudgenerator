...
<head>
	<html:base />
	<title>]]><xsl:value-of select="@name"/><![CDATA[_update.jsp</title>
		...
		<tr>
			...
			<td class="title1_td2">
				<input class="button_2zi" type="button" value="保 存"
					onclick="if(validate()) dosubmit(this.form, '../]]><xsl:value-of select="string:firstLetterLower(@clazz)"/><![CDATA[.do?method=update]]><xsl:value-of select="@clazz"/><![CDATA[Do');" />
