	...
	<xsl:template match="table">
		<root>
			...
			public class Gen<xsl:value-of select="@clazz"/>Dao {
				...
				public List&lt;Gen<xsl:value-of select="@clazz"/>&gt; get<xsl:value-of select="@clazz"/>sByPage(Gen<xsl:value-of select="@clazz"/><xsl:text> </xsl:text><xsl:value-of select="string:firstLetterLower(@clazz)"/>) {
					return (List&lt;Gen<xsl:value-of select="@clazz"/>&gt;) ibatis.queryForPage("<xsl:value-of select="parameters/parameter[@id='genPrefixLower']/@value"/><xsl:value-of select="@name"/>.get<xsl:value-of select="@clazz"/>s", <xsl:value-of select="string:firstLetterLower(@clazz)"/>);
				}
			
				public Gen<xsl:value-of select="@clazz"/> get<xsl:value-of select="@clazz"/>By<xsl:value-of select="string:firstLetterUpper(column[@isPrimary='true']/@name)"/>(<xsl:value-of select="column[@isPrimary='true']/@javaType"/><xsl:text> </xsl:text><xsl:value-of select="column[@isPrimary='true']/@name"/>) {
					return (Gen<xsl:value-of select="@clazz"/>) ibatis.queryForObject("<xsl:value-of select="parameters/parameter[@id='genPrefixLower']/@value"/><xsl:value-of select="@name"/>.get<xsl:value-of select="@clazz"/>By<xsl:value-of select="string:firstLetterUpper(column[@isPrimary='true']/@name)"/>", <xsl:value-of select="column[@isPrimary='true']/@name"/>);
				}
			
				public Integer insert<xsl:value-of select="@clazz"/>(Gen<xsl:value-of select="@clazz"/><xsl:text> </xsl:text><xsl:value-of select="string:firstLetterLower(@clazz)"/>) {
				...
				public void update<xsl:value-of select="@clazz"/>(Gen<xsl:value-of select="@clazz"/><xsl:text> </xsl:text><xsl:value-of select="string:firstLetterLower(@clazz)"/>) {
